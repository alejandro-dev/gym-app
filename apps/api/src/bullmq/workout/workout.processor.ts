...
export class WorkoutProcessor extends WorkerHost {
    ...
    */
   async process(job: Job<WorkoutCompletedJobData>): Promise<void> {
      switch (job.name) {
    ...
    */
   private async handleWorkoutCompleted(job: Job<WorkoutCompletedJobData>) {
      ...
      for (const candidate of candidates) {
         ...
         this.logger.log(
            `Nuevo PR detectado para ejercicio ${candidate.exerciseId} (${candidate.metric}=${candidate.value}) en sesión ${workoutSessionId}`,
         );
      }
   }
   ...
   private buildPersonalRecordCandidates(
      ...
      return Array.from(exerciseSetMap.entries()).flatMap(
         ([exerciseId, exerciseSets]) => {
            ...
            switch (category) {
               ...
               case ExerciseCategory.BODYWEIGHT:
                  return this.buildBodyweightCandidates(
                     exerciseId,
                     exerciseSets,
                  );
   ...
   private buildCardioCandidates(
      ...
      const maxDistance = Math.max(
         ...sets.map((set) => set.distanceMeters ?? 0),
      );
      ...
      const maxDuration = Math.max(
         ...sets.map((set) => set.durationSeconds ?? 0),
      );

...
export class AuthService {
	...
	async refreshTokens(refreshToken: string) {
		...
		if (!user?.hashedRefreshToken) throw new UnauthorizedException('Refresh token not available');
	...
	async verifyEmail(token: string) {
		...
		if (!token)	throw new BadRequestException('Token de verificación de email no proporcionado');
